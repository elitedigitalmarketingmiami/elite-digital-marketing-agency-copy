...
export default function WhyEliteSection() {
  return (
    <section className="py-24 md:py-40 relative overflow-hidden" style={{ background: '#EAE3D8' }}>
      <div className="absolute top-0 right-0 w-[500px] h-[500px] rounded-full blur-3xl pointer-events-none" style={{ background: 'rgba(214,195,163,0.25)' }} />
      <div className="absolute bottom-0 left-0 w-96 h-96 rounded-full blur-3xl pointer-events-none" style={{ background: 'rgba(214,195,163,0.15)' }} />

      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <SectionHeading
          badge="The Elite Advantage"
          title={<>The Standard Others <span className="bg-clip-text text-transparent" style={{ backgroundImage: 'linear-gradient(135deg, #C9A96E, #a8845a)' }}>Can't Reach</span></>}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
          {features.map((f, i) => (
            <motion.div
              ...
              className="group p-8 md:p-10 rounded-2xl border transition-all duration-500 hover:-translate-y-2"
              style={{ background: '#FFFFFF', borderColor: 'rgba(214,195,163,0.3)' }}
            ...
            >
              <div className="w-14 h-14 rounded-xl flex items-center justify-center mb-6 transition-transform group-hover:scale-110" style={{ background: 'rgba(201,169,110,0.1)' }}>
                <f.icon className="w-7 h-7" style={{ color: '#C9A96E' }} />
              </div>
              <h3 className="font-display font-bold text-lg mb-3 leading-snug" style={{ color: '#2B2B2B' }}>{f.title}</h3>
              <p className="text-sm font-body leading-relaxed" style={{ color: 'rgba(43,43,43,0.55)' }}>{f.desc}</p>
              <div className="mt-6 h-px w-10 rounded-full group-hover:w-20 transition-all duration-500" style={{ background: '#C9A96E' }} />
            </motion.div>
          ))}
        </div>
      </div>
